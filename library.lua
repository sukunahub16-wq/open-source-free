...
local redzlib = {
	Themes = {
		...
		White = {
			...
			["Color Edge"] = Color3.fromRGB(0, 0, 0),
			["Color Theme"] = Color3.fromRGB(186, 60, 255),
			["Color Text"] = Color3.fromRGB(0, 0, 0),
			["Color Dark Text"] = Color3.fromRGB(225, 25, 25),
			["Color Bubble"] = Color3.fromRGB(255, 105, 180)
		}
	},
	Info = {
		Version = "1.2.0"
...
local function AddNeonTitle(Label, ColorA, ColorB, SweepTime, GapTime)
	local Mixed = ColorA:Lerp(ColorB, 0.5)
	local Neon = Create("UIGradient", Label, {
		Rotation = 15,
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, ColorA),
			ColorSequenceKeypoint.new(0.35, ColorA),
			ColorSequenceKeypoint.new(0.50, Mixed),
			ColorSequenceKeypoint.new(0.65, ColorB),
	...
	task.spawn(function()
		while Label and Label.Parent do
			Neon.Offset = Vector2.new(-1.5, 0)
			CreateTween({Neon, "Offset", Vector2.new(1.5, 0), SweepTime or 1.3})
			task.wait((SweepTime or 1.3) + (GapTime or 2.2))
		end
	end)
...
function redzlib:MakeWindow(Configs)
	...
	AddNeonTitle(Title, Color3.fromRGB(186, 60, 255), Color3.fromRGB(255, 90, 200), 1.3, 2.2)
		...
		SettingsTab:AddToggle({
			...
			Callback = function(enabled)
				...
				if not enabled and Title then
					...
					local SubTitleLbl = Title:FindFirstChild("SubTitle")
					if SubTitleLbl then
						SubTitleLbl.TextColor3 = Color3.fromRGB(255, 140, 0)
					end
				end
			end
		})

...
function redzlib:SetAutoTabSearch(Bool)
	redzlib.AutoTabSearch = Bool and true or false
end
...
function Window:AddMinimizeButton(Configs)
	...
	function Window:MakeTab(paste, Configs)
		...
		local WantSearch = Configs.Search
		if WantSearch == nil then WantSearch = Configs.TabAddfevipEfect end
		if WantSearch == nil then WantSearch = redzlib.AutoTabSearch end

		if WantSearch then
			local SearchHolder = Create("Frame", Container, {
			...
			local SearchBox = InsertTheme(Create("TextBox", SearchHolder, {
				...
				BackgroundColor3 = Theme["Color Hub 2"],
				TextColor3 = Theme["Color Text"],
				PlaceholderText = Configs.SearchPlaceholder or "Search...",
			...
			function Tab:OnSearch(func)
				return Funcs:InsertCallback(Tab.SearchCallbacks, func)
			end

			SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
				Funcs:FireCallback(Tab.SearchCallbacks, SearchBox.Text:lower())
			end)
		end
		...
		function Tab:AddDropdown(Configs)
			...
			local DSearch = Configs.Search
			if DSearch == nil then DSearch = true end
			local DropWidth = Configs.Width or 160
			local Callback = Funcs:GetCallback(Configs, 4)

			local Button, LabelFunc = ButtonFrame(Container, DName, DDesc, UDim2.new(1, -DropWidth - 30))

			local SelectedFrame = InsertTheme(Create("Frame", Button, {
				Size = UDim2.new(0, DropWidth, 0, 24),
				Position = UDim2.new(1, -10, 0.5),
				AnchorPoint = Vector2.new(1, 0.5),
				BackgroundColor3 = Theme["Color Hub 2"],
				BackgroundTransparency = 0
			}), "Frame")
			local SelectedStroke = InsertTheme(Make("Stroke", SelectedFrame), "Stroke")
			Make("Corner", SelectedFrame, UDim.new(0, 7))
			...
			SelectedFrame.MouseEnter:Connect(function()
				CreateTween({SelectedStroke, "Color", Theme["Color Theme"], 0.2})
			end)
			SelectedFrame.MouseLeave:Connect(function()
				CreateTween({SelectedStroke, "Color", Theme["Color Stroke"], 0.2})
			end)
			...
			local DropFrame = Create("Frame", NoClickFrame, {
				Size = UDim2.new(0, DropWidth + 2, 0, 0),
				BackgroundTransparency = 1,
				BackgroundColor3 = Theme["Color Hub 2"],
				AnchorPoint = Vector2.new(0, 1),
				Name = "DropdownFrame",
			...
			local DropStroke = Make("Stroke", DropFrame)
			DropStroke.Transparency = 1
			Make("Gradient", DropFrame, {Rotation = 60})

			local SearchBarHeight = DSearch and 30 or 0
			local SearchBox

			if DSearch then
				local SearchHolder = Create("Frame", DropFrame, {
					Size = UDim2.new(1, -10, 0, SearchBarHeight - 8),
					Position = UDim2.new(0, 5, 0, 4),
					BackgroundTransparency = 1
				})

				SearchBox = InsertTheme(Create("TextBox", SearchHolder, {
					Size = UDim2.new(1, 0, 1, 0),
					BackgroundColor3 = Theme["Color Stroke"],
					TextColor3 = Theme["Color Text"],
					PlaceholderText = Configs.SearchPlaceholder or "Search...",
			...
			local function Disable()
				...
				CreateTween({DropFrame, "Size", UDim2.new(0, DropWidth + 2, 0, 0), 0.2})
				CreateTween({DropFrame, "BackgroundTransparency", 1, 0.15})
				CreateTween({DropStroke, "Transparency", 1, 0.15, true})
			...
			local function GetFrameSize()
				return UDim2.fromOffset(DropWidth + 2, ScrollSize)
			end

			local function CalculateSize()
				local Count = 0
				for _,Frame in pairs(ScrollFrame:GetChildren()) do
					if (Frame:IsA("Frame") or Frame.Name == "Option") and Frame.Visible ~= false then
						Count = Count + 1
					end
				end
				ScrollSize = (math.clamp(Count, 0, 8) * 25) + 10 + SearchBarHeight
			...
			local function Minimize()
				...
				if NoClickFrame.Visible then
					...
					CreateTween({DropFrame, "BackgroundTransparency", 0.05, 0.2})
					CreateTween({DropStroke, "Transparency", 0, 0.2})
					CreateTween({DropFrame, "Size", GetFrameSize(), 0.2, true})
			...
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
				...
				if SearchBox then
					SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
						...
						for _,v in pairs(Options) do
							v.nodes[1].Visible = Query == "" or v.Name:lower():find(Query, 1, true) ~= nil
						end
						CalculateSize()
					end)
				end
